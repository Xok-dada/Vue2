...
## Vuex
    ...
    6.四个map方法的使用
        ...
        (4)mapMutations方法：用于帮助我们生成与mutations对话的方法，即：包含$store.dispatch(xxx)的函数
            methods:{
...
## 路由
    ...
    11.两个新的生命周期钩子
        ...
        (2)具体名字:
            i.activated路由组件被激活时触发。
            ii.deactivated路由组件失活时触发。
    ...
    13.路由器的两种工作模式
        ...
        (3)hash模式：
            i.地址中永远带着#号，不美观 。
            ii.若以后将地址通过第三方手机app分享，若app校验严格，则地址会被标记为不合法。
            iii.兼容性较好。
        (4)history模式：
            i.地址干净，美观 。
            ii.兼容性和hash模式相比略差。
            iii.应用部署上线时需要后端人员支持，解决刷新页面服务端404的问题。
